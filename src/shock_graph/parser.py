...
from .structures import ShockGraph, Node, SamplePoint, Edge

class ShockParser:
    """Parser matching dbsk2d_xshock_graph_fileio.cxx logic."""

    TYPE_MAP = {
        'A': 'A3', 'S': 'SOURCE', 'F': 'SINK', 'J': 'JUNCT', 'T': 'TERMINAL'
    }
    ...
    def parse(self) -> ShockGraph:
        ...
        # 1. Load Nodes
        node_block = re.search(r'Begin \[NODE DESCRIPTION\](.*?)End \[NODE DESCRIPTION\]', content, re.S)
        if node_block:
        ...
        edge_block = re.search(r'Begin \[EDGE DESCRIPTION\](.*?)End \[EDGE DESCRIPTION\]', content, re.S)
        if edge_block:
            pattern = r'(\d+)\s+I\s+\[(\d+)\s+(\d+)\]\s+\[(.*?)\]'
            for m in re.finditer(pattern, edge_block.group(1)):
                e_id, u_id, v_id = int(m.group(1)), int(m.group(2)), int(m.group(3))
                s_ids = [int(i) for i in m.group(4).split()]
                
                src_node, tgt_node = graph.nodes[u_id], graph.nodes[v_id]
                edge_samples = [all_samples[sid] for sid in s_ids]

                if src_node.sample is None: src_node.sample = edge_samples[0]
                if tgt_node.sample is None: tgt_node.sample = edge_samples[-1]

                # Edge instantiation triggers degree updates in nodes
                graph.edges.append(Edge(e_id, src_node, tgt_node, edge_samples))
    ...
    def _extract_all_samples(self, content: str) -> Dict[int, SamplePoint]:
        ...
        for b in blocks:
            ...
            x, y, t = map(float, re.search(r'\(x, y, t\)\s+\((.*?)\)', b).group(1).split(','))
            speed = float(re.search(r'speed\s+([e\d\.\+\-]+)', b).group(1))
            theta = float(re.search(r'theta\s+([\d\.]+)', b).group(1)) * math.pi / 180.0
            phi = math.acos(-1.0/speed) if 0 < abs(speed) < 99990 else math.pi/2
