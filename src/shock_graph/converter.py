...
class GraphConverter:
    ...
    def _extract_raw_data(
        ...
        # Adding e.id as a tie-breaker guarantees strict mathematical ordering even for multigraphs.
        sorted_edges = sorted(
            graph.edges, 
            key=lambda e: (node_to_idx[e.source.id], node_to_idx[e.target.id], e.id)
        )
